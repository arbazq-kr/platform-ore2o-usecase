...
#            0.90 means 90% similar to be sub-grouped
...
#  Core AI function - groups incidents by semantic similarity
#  Uses cosine similarity between sentence embeddings
# ══════════════════════════════════════════════════════════

def get_ai_similarity_groups(df_subset, text_column, threshold, model):
    """
    Groups incidents by semantic similarity using AI embeddings.

    How it works:
    1. Takes all text values from text_column
    2. Converts each text to a numerical vector (embedding)
       using the AI model
    3. Computes cosine similarity between all pairs of vectors
       (cosine similarity = how similar two vectors are, 0 to 1)
    4. Groups incidents where similarity >= threshold
    5. Returns list of groups (each group = list of DataFrame indices)
    ...
    Returns:
        List of groups → each group is a list of DataFrame indices
        Example: [[0,3,7], [1,5], [2,6,8,9]]
    """

    # ─── Drop rows where text column is None/NaN ──────────
    # Cannot compute similarity on empty text
    valid_df = df_subset[df_subset[text_column].notna()].copy()

    if len(valid_df) < 2:
        # Need at least 2 incidents to form a group
        return []
    ...
    # Each text → 384-dimensional numerical vector
    # Texts with similar meaning → vectors pointing same direction
    # Example:
    #   "Server is down"        → [0.12, 0.87, 0.34, ...]
    #   "Server not responding" → [0.11, 0.85, 0.36, ...]  ← very similar!
    #   "Disk space full"       → [0.91, 0.02, 0.78, ...]  ← very different
    embeddings = model.encode(texts, show_progress_bar=False)

    # ─── Step 2: Compute cosine similarity matrix ──────────
    # similarity_matrix[i][j] = similarity score between text i and text j
    # Score of 1.0 = identical meaning
    # Score of 0.0 = completely different meaning
    # Example for 3 texts:
    #         T0    T1    T2
    #   T0 [[1.00, 0.92, 0.21],
    #   T1   [0.92, 1.00, 0.19],
    ...
    # ─── Step 3: Group incidents by similarity threshold ───
    # Use Union-Find (disjoint set) approach:
    # Start with each incident in its own group
    # If two incidents are similar enough → merge their groups
    visited = set()
    groups  = []
    ...
    for i in range(len(texts)):
        ...
        # Start a new group with incident i
        ...
        for j in range(i + 1, len(texts)):
            ...
            # Check if incident j is similar enough to incident i
        ...
        # (single incidents have nothing to link to)
...
#  TWO LAYER AI MATCHING:
#  Layer 1 → AI semantic similarity on clean_short_description
#            (broad match, threshold LAYER1_THRESHOLD)
#  Layer 2 → AI semantic similarity on clean_description
#            (exact match, threshold LAYER2_THRESHOLD)
# ══════════════════════════════════════════════════════════

def find_and_link_similar_incidents(df, model):
    """
    Finds similar incidents using two-layer AI semantic matching
    and links them as parent-child in ServiceNow.
    Layer 1 → AI groups by clean_short_description similarity
    Layer 2 → AI sub-groups by clean_description similarity
    """
    print(f"\n{'='*65}")
    print(f"🤖 Finding Similar Incidents (Two-Layer AI Matching)...")
    print(f"   Layer 1 Threshold : {LAYER1_THRESHOLD} ({int(LAYER1_THRESHOLD*100)}% similarity)")
    print(f"   Layer 2 Threshold : {LAYER2_THRESHOLD} ({int(LAYER2_THRESHOLD*100)}% similarity)")
    print(f"{'='*65}")

    # ══════════════════════════════════════════════════════
    #  LAYER 1: AI Semantic Grouping by clean_short_description
    #  Groups incidents whose short descriptions are semantically
    #  similar (same meaning even if different words)
    # ══════════════════════════════════════════════════════

    print(f"\n🔵 Running Layer 1 AI Similarity on short_description...")

    # Get AI similarity groups for Layer 1
    # Returns list of groups → each group = list of df indices
    l1_groups = get_ai_similarity_groups(
        df_subset   = df,
        text_column = 'clean_short_description',
        threshold   = LAYER1_THRESHOLD,
        model       = model
    )

    if not l1_groups:
        print("✅ No similar incidents found in Layer 1. Nothing to link!")
        return

    total_l1_groups  = len(l1_groups)
    current_l1_group = 0

    print(f"⚠️  Found {total_l1_groups} broad similar group(s) from Layer 1!")

    for l1_group_indices in l1_groups:

        current_l1_group += 1

        # ─── Get the actual DataFrame rows for this group ─
        l1_group = df.loc[l1_group_indices].copy()

        print(f"\n{'-'*65}")
        print(f"🔵  Group {current_l1_group}/{total_l1_groups} Layer 1")
        print(f"   Short Desc : '{str(l1_group.iloc[0]['clean_short_description'])[:60]}...'")
        print(f"   Total INCs : {len(l1_group)}")

        # ══════════════════════════════════════════════════
        #  LAYER 2: AI Semantic Sub-Grouping by clean_description
        #  Within each Layer 1 group, further sub-groups incidents
        #  whose full descriptions are semantically similar
        #  Higher threshold = more precise matching
        # ══════════════════════════════════════════════════
        ...
        l2_groups = get_ai_similarity_groups(
            df_subset   = l1_with_desc,
        ...
        total_l2_groups  = len(l2_groups)
        current_l2_group = 0

        print(f"\n   ✅ Found {total_l2_groups} exact match sub-group(s) in Layer 2!")

        for l2_group_indices in l2_groups:

            current_l2_group += 1

            # ─── Get actual DataFrame rows for this sub-group
            group = l1_with_desc.loc[l2_group_indices].copy()
...
def main():
    ...
    # Load once here and pass to functions that need it
    ...
    find_and_link_similar_incidents(df, model)
